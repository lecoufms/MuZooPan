    ...
    def _onBackBtn(self, window, key, *args):
        if key == 27:
            try:
                ...
                print(os.path.join('files','quiz','tela_final','pontos.json'))
                if os.path.exists(os.path.join('files','quiz','tela_final','pontuacao.json')):
                    #os.remove(os.path.join('files','quiz','tela_final','pontos.json'))
                    os.rename(os.path.join(os.path.dirname('__file__'),'files','quiz','tela_final','pontuacao.json'),os.path.join('files','quiz','tela_final','pontos.json'))
                self.stop()
        return False
