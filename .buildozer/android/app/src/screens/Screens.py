...
from src.modules import read_json, text_functions, read_pontos
...
class PremioScrenn(Screen):
    ...
    t_qtd_acertos = NumericProperty()
    questions = NumericProperty()
    resultado = NumericProperty()
    ...
    def __getpontuacao(self, name = ''):
        ...
        try:
            if os.path.exists(os.path.join('files','quiz','tela_final','pontuacao.json')):
                ...
                self.t_qtd_acertos = j_info_score['acertos']
                self.t_total_porcetagem = (self.questions - 3) * 150 + (3 * 100)
                self.resultado = (self.t_qtd_acertos * 100) / self.t_total_porcetagem
...
class quizScreen(FloatLayout):
    ...
    def arrumatudo(self):
        ...
        try:
            testet=self.list_dir[self.momento][0]
    ...
    def verifica_resposta(self):
        testetete=self.list_dir[self.momento][0]
